...
def function_1(form):
    ...
    #При необходимости блок можно раскомментировать или удалить
    '''
    ...
    print(form_values)    
    '''
    #Будем очищать файл данных    
    if("qs_type" in form and "file_name" in form and form["qs_type"].value=='w'):
        file = "../tmp/"+ form["file_name"].value
        file_stream = open(file, mode='w', encoding="utf-8", errors=None)
        file_stream.close()
        return("Файл очищен.\nФукция отработала.\n")

    #По условию ниже и другим условиям будем корректно заполнять форму
    if "file_name" not in form:
        ...
        ''')
    #Получаем из формы все данные и записываем файл при необходимости проверяем корректность ввода
    else:
        ...
        e = list() #приращение
        f = list() #баланс
        my_sum_d=0. #накапливаемиые доходы
        my_sum_r=0. #накапливаемы расходы
        increment=0. #приращения положительные ир отрицательные
        stream=0. #поток
    ...
    '''
    return "Данные записаны в файл и обработаны.\nФукция отработала.\n"

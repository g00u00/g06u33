...
def form_dictionary():
    ...
    # print("form.__dir__(): ", form.__dir__())
...
def file_write_read_json_only():
    ...
    if "000_file_name" in form_keys_list:
        ...
        list_age = list() #для формирования списка по одному из ключей
        r_stream = open(file, mode='r', encoding="utf-8")
        print ("\n\nПострочно считываем строки из ", os.environ[ "HTTP_HOST" ] , file[2:],  sep='')
        for line in r_stream.readlines():
            print(line, end='')
            ...
            print(new_dict, '\n')
            list_age.append(new_dict.get('age'))
            # list_age.append(int(new_dict.get('age')))

        print("Список: ",list_age)
        print("len(list_age): ",len(list_age))
